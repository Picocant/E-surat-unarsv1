...
<div class="row">
    <div class="col-lg-6 mx-auto">
        <h3 class="mb-4">Tambah Data Arsip Ijazah Mahasiswa</h3>
        <div class="mb-3">
                ...
                <div class="card-body">
                    @if (sizeof($students) < 1) <div class="alert alert-light-warning mb-0" role="alert">
                        Data mahasiswa kosong atau seluruh ijazah mahasiswa sudah ditambahkan, Harap <a href="{{ route('student.create') }}" class="alert-link">input data
                            mahasiswa</a> terlebih dahulu.
                </div>
                @else
                <form action="{{ route('student-certificate.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="student_id" id="student_id">
                            ...
                            <option {{ old('student_id') == $student->id ? 'selected' : '' }} value="{{ $student->id }}">{{ $student->student_number }} -
                                {{ $student->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input placeholder="Contoh format: 2021/2022" type="text" name="academic_year" id="academic_year" class="form-control" value="{{ old('academic_year') }}">
                    </div>
                    <div class="form-group">
                        <label for="file" class="form-label">File Ijazah (pdf)</label>
