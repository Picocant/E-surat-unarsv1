...
@section('content')
    <h3 class="mb-4">Edit Surat Perintah Perjalanan Dinas</h3>
    <div class="mb-3">
        <a href="{{ route('sppd-letter.show', ['sppdLetter' => $sppdLetter]) }}" class="btn btn-sm btn-primary">Kembali</a>
    </div>
    <form action="{{ route('sppd-letter.update', ['sppdLetter' => $sppdLetter]) }}" method="post">
        @csrf
        @method('put')
        <div class="row">
            <div class="col-lg-6">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="from_user_id" id="from_user_id">
                                    @foreach ($users as $fromUser)
                                        <option
                                            {{ old('from_user_id', $sppdLetter->from_user_id) == $fromUser->id ? 'selected' : '' }}
                            ...
                            <div class="form-group">
                                ...
                                <input min="0" type="number" name="budget" id="budget" class="form-control"
                                    value="{{ old('budget', $sppdLetter->budget) }}">
                            </div>
                            <div class="form-group">
                                <label for="start_date" class="form-label">Tanggal Mulai</label>
                                <input type="date" name="start_date" id="start_date" class="form-control"
                                    value="{{ old('start_date', $sppdLetter->start_date->format('Y-m-d')) }}">
                            </div>
                            <div class="form-group">
                                <label for="end_date" class="form-label">Tanggal Selesai</label>
                                <input type="date" name="end_date" id="end_date" class="form-control"
                                    value="{{ old('end_date', $sppdLetter->end_date->format('Y-m-d')) }}">
                            </div>
                            <div class="form-group">
                                <label for="destination" class="form-label">Untuk melakukan perjalanan ke</label>
                                <textarea name="destination" class="form-control"
                                    id="destination">{{ old('destination', $sppdLetter->destination) }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="purpose" class="form-label">Maksud Perjalanan</label>
                                <textarea name="purpose" class="form-control" id="purpose">{{ old('purpose', $sppdLetter->purpose) }}</textarea>
                            </div>
                            <div class="text-end">
