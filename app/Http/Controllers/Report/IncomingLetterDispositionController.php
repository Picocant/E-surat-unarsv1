...
class IncomingLetterDispositionController extends Controller
{
    ...
    {
        gate('generate-incoming-letter-disposition-report');

        return view('report.incoming-letter-disposition.index');
    }
    ...
    public function print(Request $request)
    {
        ...
        $data = IncomingLetterDisposition::select('*');
        ...
        $oldest = IncomingLetterDisposition::oldest('created_at')->first();
        ...
        $signature = base64_encode(json_encode([
            "uid" => auth()->id(),
            "details" => [
                "Jenis" => "Laporan Disposisi Surat Masuk",
                "Periode Laporan" => $period,
                "Ditandatangani Pada" => now()->isoFormat('dddd DD MMMM YYYY'),
            ]
        ]));

        return view('report.incoming-letter-disposition.print', [
            'incomingLetterDispositions' => $data->orderBy('created_at', 'ASC')->get(),
            'printBy' => auth()->user(),
