...
class SchoolTransferLetterController extends Controller
{
    ...
    {
        gate('generate-school-transfer-letter-report');

        return view('report.school-transfer-letter.index');
    }
    ...
    public function print(Request $request)
    {
        ...
        $data = SchoolTransferLetter::select('*');
        ...
        $oldest = SchoolTransferLetter::oldest('created_at')->first();
        ...
        $signature = base64_encode(json_encode([
            "uid" => auth()->id(),
            "details" => [
                "Jenis" => "Laporan Surat Keterangan Pindah Sekolah",
                "Periode Laporan" => $period,
                "Ditandatangani Pada" => now()->isoFormat('dddd DD MMMM YYYY'),
            ]
        ]));

        return view('report.school-transfer-letter.print', [
            'schoolTransferLetters' => $data->orderBy('created_at', 'ASC')->get(),
            'printBy' => auth()->user(),
